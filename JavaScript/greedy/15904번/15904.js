...
let input = 'Union of Collegiate Programming contest club Contest';
let arr = [];
let count = 0;
arr.push(input.search(/U/));
arr.push(input.indexOf('C'));
arr.push(input.search(/P/));
arr.push(input.lastIndexOf('C'));
for(let i=0;i<arr.length-1;i++){
    if(arr[i]<arr[i+1] || arr[1] != arr[3]){
        count++;
    }
}
// console.log(arr);
console.log(count);
count == 3 ? console.log('I love UCPC') : console.log('I hate UCPC');