...
조건 1, 2를 볼때 일단 카드는 인접하고, 두 카드의 합만큼 골드를 얻는 것이지 카드가 변하진 않는다.
즉, 애초에 값이 제일 높은 카드와 나머지를 다더해준 총합을 구하면 되지 않을까?

*/

// 풀이

// 먼저 input에서 제일 큰 값을 구해보자
let arrMax = Math.max(...input);

// 합을 더해줄 변수
let total = 0;

// 배열을 돌면서 max값을 제외한 나머지 값을 arrMax와 전부 더해준뒤, 그 값들을 전부 더해준다.

for(let i=0;i<input.length;i++){
    if(input[i] != arrMax){
        total += input[i] + arrMax;
    }
}
